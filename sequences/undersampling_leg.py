...
class MRISequence:
    def __init__(self, TE, TR, fov, Nx, Ny, Nz, Nslices, venc, slice_thickness, alpha, bandwidth, tbw, heart_rate,
                 ...
                 center_percent_y=25, center_percent_z=20, rf_spoil_inc=117.0,
...
if __name__ == "__main__":
    ...
    seq = MRISequence(TE=4.7e-3, TR=7.2e-3, fov=FOV, Nx=int(np.ceil(FOV[0] / RESOLUTION[0])),Ny=int(np.ceil(FOV[1] / RESOLUTION[1])),Nz=int(np.ceil(FOV[2] / RESOLUTION[2])),
                      Nslices=6, venc=VENC, slice_thickness=200e-3, alpha=10, bandwidth=1e3, tbw=2, heart_rate=TRIG_TIME,undersampling_factor=9,
                      center_percent_y=25, center_percent_z=20, rf_spoil_inc=117.0,
                      crusher_x_cycles=2, crusher_z_cycles=4)
